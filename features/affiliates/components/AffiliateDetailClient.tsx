...
import { adminCreatePayoutAction } from "@/features/payouts/actions";
...
export default function AffiliateDetailClient({ data }: AffiliateDetailProps) {
  const { affiliate, stats: initialStats, transactions, payouts: initialPayouts } = data;
  const [isActive, setIsActive] = useState(affiliate.isActive);
  const [toggling, setToggling] = useState(false);
  const [copied, setCopied] = useState(false);

  const [stats, setStats] = useState(initialStats);
  const [payoutsList, setPayoutsList] = useState<PayoutItem[]>(initialPayouts || []);

  // Modal State for Payout
  const [isPayoutModalOpen, setIsPayoutModalOpen] = useState(false);
  const [payoutAmount, setPayoutAmount] = useState("");
  const [bankName, setBankName] = useState(affiliate.bankName || "BCA");
  const [accountNumber, setAccountNumber] = useState(affiliate.bankAccountNumber || "");
  const [accountName, setAccountName] = useState(affiliate.bankAccountName || affiliate.name);
  const [proofUrl, setProofUrl] = useState("");
  const [payoutNotes, setPayoutNotes] = useState("");
  const [submittingPayout, setSubmittingPayout] = useState(false);

  // Active Tab: Transactions vs Payout History
  ...
  const handleCreatePayoutSubmit = async (e: React.FormEvent) => {
    ...
    if (amountNum > stats.availableBalance) {
      alert(`Nominal pencairan melebihi sisa saldo komisi (${formatRupiah(stats.availableBalance)}).`);
      return;
    }

    setSubmittingPayout(true);
    try {
      const res = await adminCreatePayoutAction({
      ...
      if (res.success) {
        const newPayoutItem: PayoutItem = {
          id: res.payoutId || String(Date.now()),
          amount: amountNum,
          status: "APPROVED",
          bankName,
          accountNumber,
        ...
        setPayoutsList([newPayoutItem, ...payoutsList]);
        setStats(prev => ({
          ...prev,
          totalPaidOut: prev.totalPaidOut + amountNum,
          availableBalance: Math.max(0, prev.availableBalance - amountNum),
        }));
        ...
        setProofUrl("");
        setPayoutNotes("");
        alert("Pencairan komisi berhasil diproses dan dicatat!");
      } else {
        alert(res.message);
      }
    } catch (err) {
    ...
    <div className="flex flex-col gap-6 w-full pb-10">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-gray-500 font-bold">
          Detail rincian performa penjualan, komisi, dan pencairan dana partner afiliasi.
        </p>
      </div>
      ...
      <div className="bg-[#F5F3FF] border border-[#E9D5FF]/70 rounded-2xl p-6 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 divide-y lg:divide-y-0 lg:divide-x divide-[#E9D5FF]/70">
          ...
          {/* Section 2: Total Akumulasi Komisi */}
          <div className="flex flex-col justify-between py-4 lg:py-0 lg:px-6 gap-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-purple-500">Akumulasi Komisi</span>
          ...
          {/* Section 3: Sudah Dicairkan */}
          <div className="flex flex-col justify-between py-4 lg:py-0 lg:px-6 gap-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-emerald-600">Sudah Dicairkan</span>
            ...
            <span className="text-xl font-extrabold text-[#065F46]">
              {formatRupiah(stats.totalPaidOut)}
            </span>
          </div>

          {/* Section 4: Sisa Saldo Siap Dicairkan */}
          <div className="flex flex-col justify-between py-4 lg:py-0 lg:px-6 gap-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-amber-600">Saldo Belum Dicairkan</span>
              <div className="h-8 w-8 rounded-full bg-white border border-amber-200 flex items-center justify-center text-amber-600 shadow-xs">
          ...
          {/* Section 5: Tombol Eksekusi Pencairan Komisi */}
          <div className="flex flex-col justify-center items-stretch pt-4 lg:pt-0 lg:pl-6 gap-2">
            <button
              onClick={() => {
                setPayoutAmount(stats.availableBalance > 0 ? String(stats.availableBalance) : "");
                setIsPayoutModalOpen(true);
              }}
              disabled={stats.availableBalance <= 0}
              className="w-full py-3 px-4 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-xl text-xs font-extrabold shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed active:scale-98"
            >
              <PlusCircle className="h-4 w-4" />
              <span>Cairkan Komisi</span>
            </button>
            <span className="text-[10px] text-center text-purple-700/70 font-semibold">
      ...
      {/* Tab Navigation: Riwayat Transaksi vs Riwayat Pencairan Komisi */}
      <div className="flex items-center gap-2 border-b border-gray-200">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-3 text-xs font-extrabold border-b-2 transition-all cursor-pointer ${
            activeTab === "transactions"
              ? "border-[#2C1F63] text-[#2C1F63]"
              : "border-transparent text-gray-400 hover:text-black"
          }`}
        >
          <FileText className="h-4 w-4" />
          <span>Riwayat Transaksi Rujukan ({transactions.length})</span>
        </button>
        ...
        >
          ...
          <span>Histori Pencairan Komisi ({payoutsList.length})</span>
        </button>
      </div>
        ...
        <div className="bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-5 border-b border-gray-100 bg-gray-50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-extrabold text-sm text-black uppercase tracking-tight">Riwayat Transaksi Rujukan</h2>
            </div>
        ...
        <div className="bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-extrabold text-sm text-black uppercase tracking-tight">Histori Pencairan Komisi</h2>
            </div>
            <span className="text-xs font-bold text-gray-400">Total {payoutsList.length} Pencairan</span>
          </div>

          {payoutsList.length === 0 ? (
            <div className="p-10 text-center flex flex-col items-center justify-center gap-2">
              <p className="text-sm font-bold text-gray-400">Belum ada histori pencairan komisi untuk partner ini.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                    ...
                    <th className="px-6 py-3.5 font-extrabold">Nominal</th>
                ...
                <tbody className="divide-y divide-gray-100 text-xs font-semibold text-gray-600">
                  {payoutsList.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 font-extrabold text-[#065F46]">
                        {formatRupiah(p.amount)}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="font-extrabold text-black">{p.bankName} - {p.accountNumber}</span>
                          <span className="text-[10px] text-gray-400 font-bold">a.n. {p.accountName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        {p.proofUrl ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 text-[#7C3AED] hover:underline font-bold"
                          >
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-2xl max-w-md w-full p-6 shadow-xl flex flex-col gap-4">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <h3 className="font-extrabold text-sm text-black uppercase">Cairkan Komisi Afiliasi</h3>
              <button onClick={() => setIsPayoutModalOpen(false)} className="text-gray-400 hover:text-black">
                <X className="h-4 w-4" />
              </button>
            </div>

            <div className="bg-[#F5F3FF] border border-[#E9D5FF] rounded-xl p-3 text-xs flex flex-col gap-1 text-[#2C1F63]">
              <span className="font-extrabold">{affiliate.name} ({affiliate.code})</span>
              <span className="font-bold">Sisa Saldo Siap Dicairkan: {formatRupiah(stats.availableBalance)}</span>
            </div>

            <form onSubmit={handleCreatePayoutSubmit} className="flex flex-col gap-3">
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold text-gray-700">Nominal Pencairan (Rp) *</label>
                <input
                  type="number"
                  required
                  min={10000}
                  ...
                  className="px-3 py-2 border border-gray-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-[#2C1F63] text-black"
                />
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-gray-700">Bank Tujuan</label>
                  <input
                    type="text"
                    value={bankName}
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-gray-700">No. Rekening</label>
                  <input
                    type="text"
                    value={accountNumber}
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  placeholder="Catatan transfer admin..."
                  value={payoutNotes}
                  onChange={(e) => setPayoutNotes(e.target.value)}
                  className="px-3 py-2 border border-gray-200 rounded-xl text-xs font-semibold focus:outline-none focus:border-[#2C1F63] min-h-[60px] text-black"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-xl text-xs font-extrabold shadow-sm transition-all cursor-pointer disabled:opacity-50"
                >
                  {submittingPayout ? "Memproses..." : "Konfirmasi Pencairan"}
                </button>
              </div>
            </form>
          </div>
