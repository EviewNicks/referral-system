...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setManualError("Gagal melakukan login. Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#2C1F63] flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-white border border-gray-100 rounded-3xl p-8 shadow-2xl z-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <h1 className="text-xl font-extrabold text-black tracking-tight">Selamat Datang Kembali</h1>
          <p className="text-xs text-gray-500 font-medium">Masuk ke portal admin atau akun Anda.</p>
        </div>
        ...
        <div className="text-center border-t border-gray-100 pt-4 flex flex-col gap-2">
          <p className="text-xs text-gray-500">
            Belum punya akun?{" "}
            <Link href="/register" className="font-bold text-[#2C1F63] hover:underline">
              Daftar di sini
            </Link>
          </p>
